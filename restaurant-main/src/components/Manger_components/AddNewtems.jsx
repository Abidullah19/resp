...
const AddNewItems = ({ addNewItems, getItems }) => {
  const [formData, setFormData] = useState({
    title: "",
    unitPrice: "",
    category: "",
    image: "",
    productsUsed: [], // We'll dynamically add products here
  });
  const [getCat, setGetCat] = useState();
  const [imagePreview, setImagePreview] = useState(null); // For displaying the preview of the image
  const [showStockModal, setShowStockModal] = useState(false); // For showing the stock modal
  const [productList, setProductList] = useState([]); // List of products to display in the stock modal
  ...
  const handleSubmit = async (e) => {
    ...
    const formDataToSubmit = new FormData(); // Create FormData to send file and text data
    ...
    ); // Append productsUsed data
    setloading(true)
    try {
      const response = await api.post("add-items", formDataToSubmit);
      Swal.fire({
        title: 'Success!',
  ...
  const getProducts = async () => {
    // Fetch products to be used in the stock modal
    try {
        ...
        {showStockModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
            <div className="bg-white p-8 w-[40rem] rounded-lg shadow-md border border-gray-300">
              <h3 className="text-2xl font-semibold mb-4 text-center text-gray-800">Add Stock</h3>
                    ...
                    <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="border p-4">
                        <input
                          ...
                          disabled={!formData.productsUsed.some((item) => item.productId === product._id)}
                        />
                      </td>
                    </tr>
                  ))}
